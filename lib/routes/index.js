...
module.exports = function(app){
    ...
    app.all('/demo', [
        ...
        function(req, res, next){
            ...
            if(!req.image){
                if(!req.njax.files){
                    return next(new Error("No files posted in"));
                }
    ...
    app.all('/librarys/:library/symbols/create_from_cluster', function(req, res, next){
        async.series([
            function(cb){
                //console.log("LODING DEMO USER");
                app.demoUser(function(err, account){
            ...
            function(cb){
                app.model.Image.findOne(
                    { $or: [
                        { _id: req.body.image || req.query.image },
                        { namespace: req.body.image || req.query.image },
            ...
            function(cb){
                ...
                //Create a symbol
                if(!req.query.name || !req.query.namespace){
                    return next(new Error("Need valid symbol data"));
                }
                if(req.query.namespace && !req.body.namespace){
                    req.body.namespace = req.query.namespace;
                }
                if(req.query.name && !req.body.name){
                    req.body.name = req.query.name;
                }
                app.njax.routes.symbol.create(req, res, function(err){
                    if(err) return next(err);
            ...
            function(cb){
                ...
                var cluster = null;
                for(var i = 0; i < req.image.clusters.length; i++){
                    if(req.image.clusters[i]._id == (req.body.cluster || req.query.cluster)){
